...
contract PodFiPlatform is Ownable {
  ...
  function createPodcast(string memory name) external {
    _podcastId += 1;
    uint256 newPodcastId = _podcastId;

    // Create a new Podcast struct with the initial values
    Podcast storage newPodcast = podcasts[newPodcastId];
    newPodcast.id = newPodcastId;
    newPodcast.creator = msg.sender;
    newPodcast.name = name;
    newPodcast.averageEngagement = 0;
    emit PodcastCreated(_podcastId, msg.sender);
  }
  ...
  function mintExclusiveContent(address to, string memory tokenURI) external onlyOwner {
    _podcastId += 1;
    uint256 newTokenId = _podcastId;
    // exclusiveContent._mint(to, newTokenId);
